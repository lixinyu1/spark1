...
object DeviceDevRpt {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = df.map(row => {
      ...
      var devicetypename=""
      devicetype match {
        case 1=> devicetypename="手机"
        case 2=> devicetypename="平板"
        case _=> devicetypename="其它"
      }
      // 创建三个对应的方法处理九个指标
      ((devicetypename), (RptUtils.request(requestmode, processnode) ++ RptUtils.click(requestmode, iseffective) ++ RptUtils.Ad(iseffective, isbilling, isbid, iswin,
