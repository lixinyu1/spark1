...
object AdPlatformRpt {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = df.map(row => {
      ...
      // 创建三个对应的方法处理九个指标
      ((adplatformproviderid), (RptUtils.request(requestmode, processnode) ++ RptUtils.click(requestmode, iseffective) ++ RptUtils.Ad(iseffective, isbilling, isbid, iswin,
