...
//    jobconf.set(TableOutputFormat.OUTPUT_TABLE, hbaseTableName)
    ...
    // 获取停用词库
    val stopword = sc.textFile(stopPath).map((_, 0)).collectAsMap().toArray.map(_._1)
    ...
    val vertiesRDD: RDD[(Long, List[(String, Int)])] = baseRDD.flatMap(tp => {
      ...
      val keywordList = TagsKey.makeTags(row, bcstopword.value)
      val dvList = TagsDev.makeTags(row)
